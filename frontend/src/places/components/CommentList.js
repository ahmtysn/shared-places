...
function CommentList(props) {
	...
	const commentSubmitHandler = async event => {
		...
		if (validInput) {
			...
			try {
				await sendRequest(
					`http://localhost:5000/api/comments/newComment`,
					"POST",
					JSON.stringify({
						date: commentDate,
						userId: userId,
	...
	const deleteComment = async deletedcommentId => {
		setComments(prevComment => prevComment.filter(comment => comment.id !== deletedcommentId));
		try {
			await sendRequest(
				`http://localhost:5000/api/comments/${deletedcommentId}/${userId}`,
				"DELETE",
				null,
				{
					Authorization: "Bearer " + auth.token,
			...
			{!auth.isLoggedIn ? (
				...
				<form className='formDiv' onSubmit={commentSubmitHandler}>
					<div className='form'>
						...
						/>
						{maxLength && <p style={{ color: "red" }}>Max length is 150 character.</p>}
					</div>
					<button className='bttn' disabled={commentInput.length < 5} type='submit'>
						POST COMMENT
					</button>
				</form>
			)}
